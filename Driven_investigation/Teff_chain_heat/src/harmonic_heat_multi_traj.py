...
def vv_update(xprev, vprev, acc_determine_prev, m, K, dt, xil, xir):
...
def single_trajectory(n):
    ...
    return Kin, Pot, powerL, power12, powerR, temperature, t_dis, x_t, v_t
...
if __name__ == '__main__':
    ...
    N = 2
    mass = np.array([15.0, 15.0])
    force_constants = np.array([0.0, 292905, 0.0])
    # N = 11
    # mass = np.array([12.0, 12.0, 12.0, 12.0, 12.0, 12.0, 12.0, 12.0, 12.0, 12.0, 12.0])
    # force_constants = np.array([0.0, 292905, 292905, 292905, 292905, 292905, 292905, \
    #     292905, 292905, 292905, 292905, 0.0])

    # kb = 0.001987 # for kcal/(mol K) [Charmm, OPLS], time needs to be define
    ...
    i = 0
    for Kin, Pot, powerL, power12, powerR, temperature, t_dis, x_t, v_t in p.map(single_trajectory, range(traj)):
