...
def vv_update(xprev, vprev, acc_determine_prev, dt, tstep):
    ...
    a_determine_new = 2 * D * alpha * np.exp(-alpha * x) * (np.exp(-alpha * x) - 1)
...
if __name__ == '__main__':
    ...
    while tstep < (t_size):
        ...
        if tstep > half_tsize:
            ...
            # Pothalf += D * (np.exp(-alpha * x_n_new) - 1)**2
            Pothalf += (D * (np.exp(-alpha * x_n_new) - 1)**2 + 0.5)
    ...
    with open ("TR-" +str(tempr) + time.strftime('-%m%d-%H%M%S.txt'),"w") as outputfile:
        ...
        outputfile.write("time end = {} \n".format(tEnd))
        outputfile.write("mode number = {} \n".format(Nslice))
        outputfile.write("omegaN = {} \n".format(omegaN))
        outputfile.write("Eeq = {} \n".format(Eeq))
    # ##-------------------------------
    ##-----append file----------
    sum_file = open("Eeq_gamma01.txt","a+")
    sum_file.write("{0}\t{1}\n".format(tempr, float(Eeq)))
