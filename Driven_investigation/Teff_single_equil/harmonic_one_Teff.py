...
def vv_update(xprev, vprev, acc_determine_prev, dt, tstep):
    ...
    Rn = np.sqrt(2 * gammar * Teff_list * deltaomega / np.pi) * np.cos(omegan * dt * tstep + phin)
    a_fluct = np.sum(Rn)
...
if __name__ == '__main__':
    ...
    # print(powerL / half_tsize, powerR / half_tsize)
    print((Kinhalf + Pothalf) / half_tsize)
    ...
    # plt.plot(x_t)
    # plt.plot(vx_t)
    # plt.plot(omegan, Teff(omegan, tempr))
    plt.plot(t_array, E_t)
    plt.ylabel('E(t)')
    plt.xlabel('t')

    print((time.time()-tstart)/60,'mins')
    plt.show()
