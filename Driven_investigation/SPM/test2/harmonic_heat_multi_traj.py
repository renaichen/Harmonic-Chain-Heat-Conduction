...
def vv_update(xprev, vprev, acc_determine_prev, m, K, dt, tstep, xil, xir):
    ...
    alpha = 1e-5
    time = dt * tstep
    v[-1] = amp * np.cos(omega_drive*time) * np.exp(-alpha*time**2)
...
def single_trajectory(n):
    ...
    while tstep < (t_size - 1):
        ...
        x_n_new, v_n_new, a_n_new = (vv_update(x_n_old,v_n_old, a_n_old, mass,
                                                force_constants, deltat, tstep, xil, xir))
